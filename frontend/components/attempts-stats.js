...
class AttemptsStats extends HTMLElement {
    async connectedCallback() {
        ...
        _apiCall
            .makeGetRequest('attempts/me')
            .then((res) => {
                if (res.data.length > 0) {
                    const scores = res.data.map((attempt) =>
                    ...
                    this.innerHTML =
                        '<section><p>Well stats are shown here, you would see that if you have some attempts.</p></section>';
                }
            })
            .catch((error) => {
                this.innerHTML =
                    '<section><p>Error loading your attempts stats.</p></section>';
            });
    }
}
