...
class Subjects extends HTMLElement {
    async connectedCallback() {
        ...
        _apiCall
            .makeGetRequest('subjects')
            .then((res) => {
                if (res.data.length > 0) {
                    const subjectsHtml = res.data
                        .map(
                            (subject) => `
                        <subject-component 
                        name="${subject.name}"
                    ...
                    this.innerHTML =
                        '<section class="subjects-container">No subjects available.</section>';
                }
            })
            .catch((error) => {
                this.innerHTML =
                    '<section class="subjects-container">Error loading subjects.</section>';
            });
    }
}
