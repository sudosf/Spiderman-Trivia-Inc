        ...
        _apiCall
            ...
            .then((res) => {
                if (res.data.length > 0) {
                    const attemptsHtml = res.data
                        .map(
                            ({ timestamp, score, subjects }) => `
                        <tr>
                            <td>${subjects.name}</td>
                            <td>${score}</td>
                    ...
                    this.innerHTML =
                        '<section><p>No attempts available. Go and play and come back here.</p></section>';
                }
            })
            .catch((error) => {
                this.innerHTML =
                    '<section><p>Error loading your attempts.</p></section>';
            });
    }
}
