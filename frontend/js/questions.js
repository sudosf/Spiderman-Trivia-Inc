...
document.addEventListener('click', (event) => {
    if (
        ...
        if (
            ...
            parentElement.href.includes('quiz')
        ) {
            _apiCall
                .makeGetRequest(`quiz/start/${getSubjectId()}`)
                .then((response) => {
                ...
                if (clickedButtonId == 'submit' && !isAnswered) {
                    if (
                        ...
                        questions[questionIndex].options[
                            BUTTONS_IDs.indexOf(currentAnswer)
                        ...
                        asideElement.insertAdjacentElement(
                            'afterend',
                            imgElement,
                        );
                        score += 1;
                        ...
                        const wrongBtn = document
                            ...
                            .querySelector('button');
                        const asideElement =
                            wrongBtn.querySelector('.btn-left');
                        const imgElement = document.createElement('img');
                        imgElement.classList.add('btn-right');
                        ...
                        const correctAnswerIndex = questions[
                            questionIndex
                        ].options.indexOf(questions[questionIndex].answer);
                        const correctAnserId = BUTTONS_IDs[correctAnswerIndex];
                        const correctBtn =
                            document.getElementById(correctAnserId);
